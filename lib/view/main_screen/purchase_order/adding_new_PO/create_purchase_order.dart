...
import 'package:calicut_textile_app/view/main_screen/purchase_order/adding_new_PO/duplicte_po_storage.dart';
...
class _CreatePurchaseOrderState extends State<CreatePurchaseOrder>
  ...
  bool _isDialogInCreationMode = false;
  ...
  void _updateSelectedUOM(String selectedUOM) {
    ...
    print('UOM Updated to: $_selectedUOM');
  }
  ...
  Future<void> _loadSavedData() async {
    // Load items
    final savedItems = await PurchaseOrderStorage.loadItems();
    ...
    // Load supplier
    final savedSupplier = await PurchaseOrderStorage.loadSupplier();
    if (savedSupplier != null) {
      setState(() {
        selectedSupplierId = savedSupplier['id'];
        selectedSupplierName = savedSupplier['name'];
        suppliercontroller.text = savedSupplier['name']!;
      });
    }

    // Load required date
    final savedDate = await PurchaseOrderStorage.loadRequiredDate();
    if (savedDate != null) {
      setState(() {
        requireddatecontroller.text = savedDate;
  ...
  void _autoSaveData() {
    PurchaseOrderStorage.saveItems(items);
    PurchaseOrderStorage.saveSupplier(selectedSupplierId, selectedSupplierName);
    PurchaseOrderStorage.saveRequiredDate(requireddatecontroller.text);
  }
  ...
  void _saveEdit() async {
    if (_editFormKey.currentState!.validate() && _editingIndex != null) {
      // UOM Validation for edit form
      if (_editSelectedUOM.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Please select UOM (NOS or METER)'),
  ...
  Widget _buildEditImageSection() {
    return _buildEditFormField(
      label: 'Images',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Current images display
          if (_editSelectedImagePaths.isNotEmpty) ...[
            SizedBox(
              height: 80,
              child: ListView.builder(
                scrollDirection: Axis.horizontal,
                itemCount: _editSelectedImagePaths.length,
                    ...
                    child: Stack(
                      children: [
                        ClipRRect(
                          ...
                          child: Image.file(
                            File(_editSelectedImagePaths[index]),
                            width: 80,
                            height: 80,
                            fit: BoxFit.cover,
                          ),
                        ...
                        Positioned(
                          ...
                          child: GestureDetector(
                            onTap: () {
                              setState(() {
                                _editSelectedImagePaths.removeAt(index);
                              });
                            },
                            child: Container(
          ...
          ElevatedButton.icon(
            ...
            label: Text('Add Images (${_editSelectedImagePaths.length})'),
            style: ElevatedButton.styleFrom(
            ...
            children: [
              ListTile(
                ...
                onTap: () async {
                  ...
                  if (images.isNotEmpty) {
                    setState(() {
                      _editSelectedImagePaths.addAll(
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  if (image != null) {
                    setState(() {
                      _editSelectedImagePaths.add(image.path);
                    });
                  }
                  Navigator.pop(ctx);
                },
        ...
        Expanded(
          child: GestureDetector(
            onTap: () {
              ...
              print('Edit Selected UOM: $_editSelectedUOM');
            },
            child: Container(
              decoration: BoxDecoration(
                color: _editSelectedUOM == 'NOS'
                    ? Colors.blue.shade600
  ...
  // Searchable dropdown methods for edit form
                        ...
                        children: [
                          ...
                          Flexible(
                            child: DialogBoxItems(
                              ...
                              onNewItemCreated: _handleNewItemCreation,
                            ),
                          ),
  ...
  void _duplicateItem(int index) {
    ...
    // Clear images from duplicated item
    duplicatedItem.imagePaths = [];
    duplicatedItem.imageCount = 0;
